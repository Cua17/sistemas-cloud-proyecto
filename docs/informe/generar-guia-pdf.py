...
OUT = r"C:\Users\jdcua\OneDrive\Universidad\2026 - Ciclo 2\Sistemas Cloud\docs\informe\Guia-NubeUltima.pdf"
...
def on_page(canvas, doc):
    ...
    canvas.drawString(2*cm, 1.1*cm, "NubeUltima - Guia del proyecto")
...
story += [Spacer(1, 4*cm),
          ...
          Spacer(1, 0.6*cm),
          Paragraph("Guia del proyecto para el grupo", ParagraphStyle(
...
story += [port, Spacer(1, 1*cm),
          Paragraph("Este documento explica, de forma sencilla, QUE construimos, COMO "
                    "funciona y COMO explicarlo en la presentacion.", SMALL)]
...
# ---------- 6 que decir ----------
story += [Paragraph("6. Que decir en la presentacion", H1),
          Paragraph("No hace falta explicar 30 herramientas. Son <b>5 frases</b>, una por "
                    "caja del diagrama:", BODY)]
frases = [
...
story += [ListFlowable([ListItem(Paragraph(p, BODY), value=i+1) for i, p in enumerate(frases)],
                       bulletType="1", leftIndent=14)]
story += [Spacer(1, 3*mm),
          Table([[Paragraph("<b>Frase de cierre:</b> \"Demostramos las 4 capas de servicio "
...
# ---------- 9 que falta ----------
story += [Paragraph("9. Que falta hacer (tareas del grupo)", H1)]
falta = [
    "Completar el <b>informe escrito</b> (borrador en el repo, carpeta docs/informe): llenar "
    "las partes marcadas, redibujar el diagrama, escribir el analisis en nuestras palabras.",
    "Sacar <b>capturas de pantalla</b> con la Pi encendida para el informe.",
    "Ejecutar el <b>simulacro de desastre</b> completo (guion en pruebas/simulacro-desastre.md) "
    "y anotar el tiempo de recuperacion.",
    "Confirmar con el profe: que son \"los 4 servicios\" y si se puede usar una Pi de 8 GB.",
    "Agregar a los integrantes como colaboradores del repositorio.",
    "Ensayar la demo.",
]
story += [ListFlowable([ListItem(Paragraph(p, BODY)) for p in falta],
...
doc = BaseDocTemplate(OUT, pagesize=A4,
                      ...
                      title="NubeUltima - Guia del proyecto",
